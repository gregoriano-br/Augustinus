...
function generateGabcNotation() {
	...
	const SHOULD_REMOVE_NUMBERS = true;
	const SHOULD_ADD_OPTIONAL_END = true;
	const SHOULD_ADD_OPTIONAL_START = true;
	const SHOULD_REMOVE_NEWLINE = true;
	const SHOULD_REMOVE_PARENTESIS = true;
	const LATEX_MODE = true;
	...
	function processTextFragmentForGabc(textFragment, state) {
		...
		function processSingleFragment(currentText, state) {
			...
			let processedTextFinal = separatedWords.join(" ").replaceAll(" ", `${syllableSeparator} `).replaceAll(`${syllableSeparator}${syllableSeparator}`, syllableSeparator);
			...
			while (remainingSymbols.length > 0) {
				...
				if (currentSymbol.match(TONIC_NOTE_REGEX)) {
					...
					singleFragmentGabc += remainingSyllables[remainingSyllables.length - tonicSyllablePosition] + currentSymbol;
					...
					if (tonicSyllablePosition == 1) {
						...
						for (let i = 1; i < syllablesToInsert.length; i++) {
							singleFragmentGabc += syllablesToInsert.shift() + middleNoteSymbol;
						}
						singleFragmentGabc += syllablesToInsert.shift() + finalNoteSymbol;
					}
					remainingSymbols.shift();
					remainingSymbols.shift();
					...
					singleFragmentGabc += " " + remainingSymbolsString;
					hasLastNote = true;
					break;
				}
					...
					if (hasGenericNote) {
						...
						singleFragmentGabc += remainingSyllables.shift() + currentSymbol;
					}
					remainingSymbols.shift();
				}
				else if (!hasLastNote) {
			...
			while (remainingSyllables.length > preTonicNotes.length) {
				let placeholderIndex = singleFragmentGabc.match(GENERIC_NOTE_REGEX).index;
				singleFragmentGabc = singleFragmentGabc.slice(0, placeholderIndex) + remainingSyllables.shift() + genericNotePlaceholder + singleFragmentGabc.slice(placeholderIndex);
			}
			while (remainingSyllables.length > 0) {
				let placeholderIndex = singleFragmentGabc.match(GENERIC_NOTE_REGEX).index;
				singleFragmentGabc = singleFragmentGabc.slice(0, placeholderIndex) + remainingSyllables.shift() + preTonicNotes.shift() + singleFragmentGabc.slice(placeholderIndex);
			}
			singleFragmentGabc = singleFragmentGabc.replace(/([a-h])\1/gm, "$1").replace(GENERIC_NOTE_REGEX, "").replace(GENERIC_NOTE_REGEX, "").replace("'", " (,) ");
			singleFragmentGabc += " ";
			return singleFragmentGabc;
		}
		...
		for (let fragmentIndex = 0; fragmentIndex < TEXT_FRAGMENTS.length - 1; fragmentIndex++) {
			...
			fragmentGabc += processSingleFragment(currentText, state);
		}
		return fragmentGabc;
	}
